...
// Pattern import removed as IMG_SRC_REGEX is removed
...
data class RssPostEntry(
    ...
    val link: String,
    val pubDate: String, 
    val description: String, // This will be the plain text version
    val imageUrl: String? // Will always be null
)
...
class PostsSyncWorker(private val appContext: Context, workerParams: WorkerParameters) :
    ...
    companion object {
        ...
        // IMG_SRC_REGEX removed
    }

    override suspend fun doWork(): Result {
        Log.d(TAG, "Starting RSS Posts sync work (images disabled)")
        ...
        return if (newPostsToProcess.isNotEmpty()) {
            ...
            newPostsToProcess.forEach { postEntry ->
                try {
                    ...
                    Log.d(TAG, "Inserted new post to DB - GUID: ${dbPost.guid}, Title: ${dbPost.title}, ImageURL: null")
                    processedGuids.add(postEntry.guid) 
                } catch (e: DateTimeParseException) {
    ...
    private fun fetchAndParseRssFeed(): List<RssPostEntry> {
        ...
        try {
            ...
            var currentRawDescriptionHtml: String? = null // Keep for Html.fromHtml
            // currentImageUrl variable removed
            var inItem = false
                ...
                when (eventType) {
                    XmlPullParser.START_TAG -> {
                        if (tagName.equals("item", ignoreCase = true)) {
                            ...
                            currentLink = null
                            currentPubDate = null
                            currentRawDescriptionHtml = null 
                        } else if (inItem) {
                            when {
                                tagName.equals("title", ignoreCase = true) -> currentTitle = readNextText(parser)
                                tagName.equals("link", ignoreCase = true) -> currentLink = readNextText(parser)
                                tagName.equals("pubDate", ignoreCase = true) -> currentPubDate = readNextText(parser)
                                tagName.equals("guid", ignoreCase = true) -> currentGuid = readNextText(parser)
                                tagName.equals("description", ignoreCase = true) -> {
                                    currentRawDescriptionHtml = readNextText(parser) // Read description as simple text
                                }
                                else -> skip(parser) 
                            }
                        }
                    ...
                    XmlPullParser.END_TAG -> {
                        if (tagName.equals("item", ignoreCase = true) && inItem) {
                            val plainTextDescription = if (currentRawDescriptionHtml != null) {
                                Html.fromHtml(currentRawDescriptionHtml, Html.FROM_HTML_MODE_LEGACY).toString().trim()
                            } else {
                                ""
                            }
                            if (currentGuid != null && currentTitle != null && currentLink != null && currentPubDate != null) {
                                entries.add(RssPostEntry(currentGuid, currentTitle, currentLink, currentPubDate, plainTextDescription, null))
                            } else {
    ...
    private fun readNextText(parser: XmlPullParser): String {
        return try {
            ...
            Log.e(TAG, "Error in readNextText for tag ${parser.name}", e)
            ""
        } catch (e: IOException) {
            Log.e(TAG, "Error in readNextText for tag ${parser.name}", e)
            "" 
    ...
    // readHtmlContentRobust removed

    private fun skip(parser: XmlPullParser) {
        if (parser.eventType != XmlPullParser.START_TAG) {
            // This should ideally not happen if readNextText correctly consumes the END_TAG.
            // If it does, we might be in an inconsistent state or at an END_TAG already.
            Log.w(TAG, "Skip called when parser not at START_TAG. Current event: ${parser.eventType}, Name: ${parser.name}, Text: '${parser.text?.take(30)}'.")
            return // Exit skip if not at a start tag to prevent errors.
        }
        var depth = 1
        while (depth != 0) {
            when (parser.next()) {
                ...
                XmlPullParser.END_DOCUMENT -> {
                    Log.e(TAG, "Skip reached END_DOCUMENT prematurely.")
                    return 
                }
            }
        }
